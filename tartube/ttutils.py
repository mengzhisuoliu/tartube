...
def convert_youtube_to_invidious(app_obj, url):
    ...
    if re.search(r'^https?:\/\/(www\.)+youtube\.com', url) \
    and re.search(r'\w+\.\w+', app_obj.custom_invidious_mirror):
...
def extract_livestream_data(stderr):
    ...
    match_list = re.search(
        r'This live event will begin in (\d+) minute',
        stderr,
    )
    ...
    match_list = re.search(
        r'This live event will begin in (\d+) hour',
        stderr,
    )
    ...
    match_list = re.search(
        r'This live event will begin in (\d+) day',
        stderr,
    )
...
classic_flag):
    ...
    mod_options_list.append('ffmpeg')
    mod_options_list.append('--external-downloader-args')
    mod_options_list.append('ffmpeg:' + stamp_arg)
...
def stream_output_is_ignorable(stderr):
    ...
    match = re.search(r'^(frame.*speed\=\s*[\S]+x)', stderr)
    if match:
